...
ギルドで相手ごとに挑む回数を決めておくと、額縁の中でバトルが自動的に進みます。
眺めているあいだに、その日の戦いが少しずつ消化されていく仕組みです。

<div class="step-list">
<ol>
<li>下部メニューから「ギルド」を開き、右上の ⚙️ 歯車から「オートバトル」をオンにする</li>
<li>相手カードに現れた回数の欄で、挑む回数を設定する</li>
<li>ホーム画面に戻ると、設定した相手から順に挑戦が始まります</li>
</ol>
</div>

1 戦ごとに残り回数が 1 つ減り、0 になったところで止まります (勝敗にかかわらず 1 回と数えます)。
額縁でのバトルは最大 3x の速度で進みますので、攻撃の様子はゆっくりご覧いただけますよ。
お休みになりたいときは、同じ歯車からオフに切り替えてください 🪶
...
「習慣を追加」「習慣を編集」画面では、各項目の左端にあるハンドル (⣿) を長押しして、
お好きな順に並び替えていただけます。実際にこなす順に並べておくと、朝の支度などが追いやすくなりますね。
...
**バトル設定 (オートバトル / 速度 / スキップ)**

ギルド画面右上の **⚙️ 歯車** から、バトルにまつわる設定をまとめて開けます。

| 設定 | 内容 |
|---|---|
| **オートバトル** | ホーム画面の額縁でバトルを自動で進めるかどうか (詳しくは[額縁のオートバトル](#額縁のオートバトル)) |
| **バトルスピード** | 1x / 1.5x / 2x / 3x / ⏭ (スキップ) の 5 段階 |

1x 以外を選んでいるあいだは、歯車の左に現在の速度が小さく表示されます。
モーダルを開かなくても今の設定が分かりますので、等速に戻したいときの目印にしてください。

戦闘中に変えたくなった場合は、バトル画面の上部からも同じ 5 段階を切り替えていただけますよ 🪶
...
Sabiowl は **日本語と英語**でお使いいただけます。

<div class="step-list">
<ol>
<li>メニュー (☰) → 「設定」を開く</li>
<li>「言語」セクションの「表示言語」をタップ</li>
<li>「日本語」「English」からお選びいただく</li>
</ol>
</div>

選択はすぐに画面へ反映され、次回起動時も保たれます。
はじめてお使いになる際は端末の言語を参照いたしますので、
端末が英語設定であれば、最初から英語で立ち上がります。

画面の文言だけでなく、通知・敵やキャラクターの名前・チャレンジの内容も切り替わります。
本サイトの各ページも、上部のリンクから English 版へ移動していただけますよ 🪶
