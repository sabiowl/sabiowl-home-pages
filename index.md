---
layout: page_with_lang
title: Sabiowl
---

# Sabiowl

**Sabiowl** は、日々の習慣・タスク・予定をゲーム的な楽しさで継続支援する
iOS 向けのモバイルアプリです。習慣を記録するたびに経験値を獲得し、
キャラクターがレベルアップしていく RPG 風の成長体験で、継続のモチベーションを
維持できます。

本ページは Sabiowl アプリの公式サイトです。アプリの機能紹介、法的文書、
お問い合わせ先、リリースノート等をまとめています。

## Sabiowl の主な機能

- **習慣の記録と可視化** — 毎日の習慣を記録し、達成状況をタイムラインで確認できます。
- **RPG 風の成長要素** — 習慣達成で経験値を獲得、キャラクターがレベルアップし
  6 種類のステータス (運動・学習・健康・精神・創造・貢献) が成長していきます。
- **Google カレンダー連携** — Google カレンダーの予定を取り込み、習慣・ToDo と
  同じタイムライン上で 1 日を統合表示できます (取り込みのみ、端末内保存)。
- **キャラクターとガチャ** — 複数のキャラクターから選択、ゲーム性のあるモチベーション設計。
- **バトルシステム** — 蓄積した習慣ステータスで敵を倒す RPG 要素。
- **眠る世界** — 習慣を達成するたびに景色の「かけら」が集まり、すべて色づくとその景色が
  動き出します。積み重ねが目に見える形で残る場所です。
- **月次チャレンジ** — 毎月のカテゴリ目標に、全ユーザーで一緒に挑めます。特別な操作は不要で、
  対象カテゴリの習慣を達成すれば自動的に加算されます。
- **日本語 / 英語表示** — 「設定 → 表示言語」からいつでも切り替えていただけます (v1.1.0 より)。

## アプリ名

- 正式名称: **Sabiowl** (サビオウル)
- マスコット: サビ (Sabi、フクロウの紳士キャラクター)

---

## 📄 ドキュメント

- 📖 [使い方ガイド](help.html) — 各画面の使い方を動画と手順で解説
- ❓ [よくあるご質問](faq.html) — 課金・トラブル対応・アカウント運用の Q&A
- 📝 [リリースノート](release_notes.html) — 各バージョンの主な変更点
- 🪶 [プライバシーポリシー](privacy_policy.html) — 個人情報・利用情報の取り扱いについて
- 📜 [利用規約](terms_of_service.html) — 本サービスのご利用条件について
- 🛒 [特定商取引法に基づく表示](specified_commercial_transactions.html) — アプリ内課金商品の販売事業者情報・返品規定等

### English

**[🌏 English site](/en/)** — app overview, plus the documents below

- 🪶 [Privacy Policy](privacy_policy_en.html) — how we handle your personal and usage information
- 📜 [Terms of Use](terms_of_service_en.html) — the conditions for using Sabiowl
- 🛒 [Legal & Consumer Information](legal_consumer_info_en.html) — who provides the app, prices, delivery, and refunds

---

## 🌐 関連リンク

- **アプリ本体**: [App Store](https://apps.apple.com/jp/app/sabiowl-%E7%BF%92%E6%85%A3rpg/id6772130388) にて配信中（iOS 15.0 以降）
- **公式 X (旧 Twitter)**: [@sabiowlapp](https://x.com/sabiowlapp) — 最新情報・更新情報をお届けします
- **お問い合わせ**: アプリ内「設定 → お問い合わせ」よりご連絡ください

---

<p style="color: #666; font-size: 0.9em; text-align: center; margin-top: 40px;">
  © 2026 Sabiowl. All rights reserved.
</p>
